...
from ogl import OpenGLObjects as objects
from ogl import OpenGLState as state
from ogl import OpenGLProperties as properties
from ogl import OpenGLCapabilities as capabilities
from ogl import OpenGLRenderLists as renderLists
from ogl import OpenGLBackground as background
from ogl import OpenGLPrograms as programCache
from ogl import OpenGLIndexedBufferRenderer as indexedBufferRenderer
from ogl import OpenGLBufferRenderer as bufferRenderer
from ogl import OpenGLAttributes as attributes
from ogl import OpenGLTextures as textures
from ogl import OpenGLLights as lights
from ogl import OpenGLUtils as utils

from ogl.openGLUniforms import OpenGLUniforms
...
def reset():
    ...
    global _currentRenderTarget, _currentFramebuffer, _currentMaterialId, \
        _currentGeometryProgram, _currentCamera, _currentViewport, _currentScissor, _currentScissorTest
...
def readRenderTargetPixels( renderTarget, x, y, width, height, buffer ):

    if not ( renderTarget and hasattr( renderTarget, "isOpenGLRenderTarget" ) ):

        logging.error( "THREE.OpenGLRenderer.readRenderTargetPixels: renderTarget is not THREE.OpenGLRenderTarget." )
        return

    framebuffer = properties.get( renderTarget )._openglFramebuffer
    ...
    if framebuffer:
        ...
        try:
            ...
            if textureFormat != RGBAFormat and utils.convert( textureFormat ) != glGetParameter( GL_IMPLEMENTATION_COLOR_READ_FORMAT ):
                ...
                logging.error( "THREE.OpenGLRenderer.readRenderTargetPixels: renderTarget is not in RGBA or implementation defined format." )
                return

            if textureType != UnsignedByteType and utils.convert( textureType ) != glGetParameter( GL_IMPLEMENTATION_COLOR_READ_TYPE ) and \
                not ( textureType == FloatType and ( extensions.get( "OES_texture_float" ) or extensions.get( "ARB_color_buffer_float" ) ) ) and \
                not ( textureType == HalfFloatType and extensions.get( "EXT_color_buffer_half_float" ) ):

                logging.error( "THREE.OpenGLRenderer.readRenderTargetPixels: renderTarget is not in UnsignedByteType or implementation defined type." )
                return
            ...
            if glCheckFramebufferStatus( GL_FRAMEBUFFER ) == GL_FRAMEBUFFER_COMPLETE:
                ...
                logging.error( "THREE.OpenGLRenderer.readRenderTargetPixels: readPixels from renderTarget failed. Framebuffer not complete." )

        finally:

            if restore:

                glBindFramebuffer( GL_FRAMEBUFFER, _currentFramebuffer or 0 )
